  ...
  if (results.length === 0) {
    return (
      <div className="text-center py-16">
        ...
        <p className={`text-lg ${hg ? 'text-gray-500' : 'text-slate-400'}`}>No results to display</p>
        <p className={`text-sm mt-1 ${hg ? 'text-gray-400' : 'text-slate-500'}`}>Run a search to find properties</p>
      </div>
    );
  }
            ...
            <CardContent className="p-4 pt-3">
              ...
              {result.externalUrl && (
                <a
                  ...
                  className={`mt-4 inline-flex items-center gap-1.5 text-sm font-medium transition-colors group/link ${
                    hg
                      ? 'text-[#e5007d] hover:text-[#ae0061]'
